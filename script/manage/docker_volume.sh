...
# ==================================================================
#
# 与 `podman_volume.sh` 是同一套逻辑，命令换成 docker：
#
#   * 删卷走规范的完整流程（打全名 · `--yes` 无效 · dry-run 可预演）
#   * **删容器不删卷**（`oneserver docker rm` 已经是这么做的），
#     卷要单独删 —— 这是有意的两步，不是遗漏
#   * 卷在用时拒绝删，不给 `--force`：docker 自己的 `volume rm -f` 会报错
#     而不是像 podman 那样连容器一起停，但拒绝仍然是更清楚的行为——
#     不给用户一个「加个 -f 就能绕过」的印象
#
# 卷也**不进任何组件的资源清单**（规范的「永不自动删除」明列了数据）。
#
# docker 没有 `volume exists` 这个子命令，改用 `docker volume inspect`
# 的退出码判断存在性，效果等价（同 `docker_image.sh` 的处理）。
...
# 哪些容器在用这个卷。`docker ps --filter volume=` 认命名卷，也认宿主路径
volume_users() {
    local __dv_out=${1} __dv_name=${2}
    os::query --timeout 20 -- docker ps -a --filter "volume=${__dv_name}" --format '{{.Names}}'
    printf -v "${__dv_out}" '%s' "${OS_RUN_OUTPUT}"
    return 0
}
...
action_create() {
    ...
    if os::query --timeout 10 -- docker volume inspect "${name}"; then
    ...
    os::defer docker volume rm "${name}"
    os::run '创建卷' -- docker volume create "${name}" || os::die 1 "创建失败：${name}"

    # dry-run 下 os::run 被跳过，卷根本没建出来——下面 docker volume inspect
    # 是只读的 os::query，dry-run 照常执行，对一个不存在的卷只会拿到空输出，
    # 而不看 OS_RUN_SKIPPED 就打「已创建」+ changed=yes，正是「会撒谎的
    # dry-run」（同 podman_volume.sh 的 create）
    if [[ ${OS_RUN_SKIPPED} -eq 1 ]]; then
        os::output 0 name="${name}" changed=dry-run
        return 0
    fi

    os::query --timeout 10 -- docker volume inspect "${name}" --format '{{.Mountpoint}}'
    os::ok "卷 ${name} 已创建"
    os::kv '挂载点' "${OS_RUN_OUTPUT:-未知}"
    os::info "用它：oneserver docker run --run-cmd='docker run -d --name db ... -v ${name}:/var/lib/data ...'"
...
action_rm() {
    ...
    # **在用就拒绝，不给 --force**：同 podman 一侧的理由，不把「删一个卷」
    # 悄悄升级成「停一个正在服务的容器」
    local users=''
    volume_users users "${name}"
    if [[ -n ${users} ]]; then
    ...
    os::run '删除卷' -- docker volume rm "${name}" || os::die 1 "删除失败：${name}"
    os::ok "卷 ${name} 已删除"
